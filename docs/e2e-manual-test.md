...
1. トレイメニューで `Settings > Language > Japanese`（または `設定 > 言語 > 日本語`）を選択
2. 設定画面を表示して文言が日本語になることを確認
3. `Recent Logs` ウィンドウを表示して文言が日本語になることを確認
4. トレイメニューで `Settings > Language > English`（または `設定 > 言語 > English`）を選択

期待:
- 設定画面と `Recent Logs` ウィンドウの文言が即時に英日で切り替わる
- 選択中の言語がトレイメニューで `✓` 表示される
